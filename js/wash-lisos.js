/* ═══════════════════════════════════════════════════════════════
   WASH LISOS — js/wash-lisos.js
   Configurações de lavagem específicas para tecidos da categoria LISOS.
   ═══════════════════════════════════════════════════════════════ */

const WASH_LISOS = {
  ATLETIKA:                [...WASH_DEFAULT],
  CIRRE:                  [...WASH_DEFAULT],
  CORTA_VENTO_AMASSADO:     [...WASH_DEFAULT],
  CORTA_VENTO_REFLETIVO:    [...WASH_DEFAULT],
  DEMI:                    [
   {img:'images/lavagem/LAVAGEM-MANUAL.jpg',        label:'LAVAR DE FORMA MANUAL'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'},
    {img:'images/lavagem/NAO-SECAR-SECADORA.jpg',    label:'NÃO SECAR EM MÁQUINA SECADORA'},
    {img:'images/lavagem/SECAR-VARAL.jpg',           label:'SECAR EM VARAL, À SOMBRA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
  ],

  DRY_FIT:                 [
     {img:'images/lavagem/LAVAGEM-MANUAL.jpg',        label:'LAVAR DE FORMA MANUAL'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'},
    {img:'images/lavagem/NAO-SECAR-SECADORA.jpg',    label:'NÃO SECAR EM MÁQUINA SECADORA'},
    {img:'images/lavagem/SECAR-VARAL.jpg',           label:'SECAR EM VARAL, À SOMBRA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
  ],
  DRY_FIT_LISTRADO:         [...WASH_DEFAULT],
  DRY_SPORT:                [...WASH_DEFAULT],
  DRY_TECH:                 [...WASH_DEFAULT],
  FLICKER_METALIZADO:        [
    {img:'images/lavagem/LAVAGEM-MANUAL.jpg',        label:'LAVAR DE FORMA MANUAL'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'},
    {img:'images/lavagem/NAO-SECAR-SECADORA.jpg',        label:'NÃO SECAR EM MÁQUINA SECADORA'},
    {img:'images/lavagem/SECAR-VARAL.jpg',           label:'SECAR EM VARAL, À SOMBRA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
  ],

  FORRO_COLMEIA:            [...WASH_DEFAULT],
  FORRO_TRADICIONAL:        [...WASH_DEFAULT],
  LIRUS: [
    {img:'images/lavagem/LAVAGEM-MANUAL.jpg',        label:'LAVAR DE FORMA MANUAL'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
    {img:'images/lavagem/NAO-SECAR-SECADORA.jpg',    label:'NÃO SECAR EM MÁQUINA SECADORA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/SECAR-VARAL.jpg',           label:'SECAR NA HORIZONTAL'},
  ],
  LUNA: [
    {img:'images/lavagem/LAVAGEM-MANUAL.jpg',        label:'LAVAR DE FORMA MANUAL'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
    {img:'images/lavagem/NAO-SECAR-SECADORA.jpg',    label:'NÃO SECAR EM MÁQUINA SECADORA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/SECAR-VARAL.jpg',           label:'SECAR NA HORIZONTAL'},
  ],
  LYCRA_5058:                [...WASH_DEFAULT],
  LYCRA_5371:                [...WASH_DEFAULT],
  MICROFIBRA_JERSINYL:          [...WASH_DEFAULT],
  NAGOYA:                  [...WASH_DEFAULT],
  NAKAY:                   [...WASH_DEFAULT],
  NEW_TRIP: [
    {img:'images/lavagem/MAX-30-1.jpg',              label:'LAVAR EM TEMPERATURA MÁX. 30°C, SUAVE'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'},
    {img:'images/lavagem/SECAR-MIN.jpg',             label:'POSSIVÉL SECAR EM MÁQUINA, TEMPERATURA BAIXA'},
    {img:'images/lavagem/SECAR-VARAL.jpg',           label:'SECAR EM VARAL, À SOMBRA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
    {img:'images/lavagem/UMIDO.jpg',                 label:'LIMPEZA A ÚMIDO'},
  ],
  NEW_ZEALAND: [
    {img:'images/lavagem/MAX-30-1.jpg',              label:'LAVAR EM TEMPERATURA MÁX. 30°C, SUAVE'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'}, 
    {img:'images/lavagem/SECAR-MIN.jpg',             label:'POSSIVÉL SECAR EM MÁQUINA, TEMPERATURA BAIXA'},
    {img:'images/lavagem/SECAR-VARAL.jpg',           label:'SECAR EM VARAL, À SOMBRA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
    {img:'images/lavagem/UMIDO.jpg',                 label:'LIMPEZA A ÚMIDO'},
  ],
  SANTA_CONSTANCIA:         [...WASH_DEFAULT],
  SATURNO:              [
    {img:'images/lavagem/LAVAGEM-MANUAL.jpg',        label:'LAVAR DE FORMA MANUAL'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'},
    {img:'images/lavagem/NAO-SECAR-SECADORA.jpg',    label:'NÃO SECAR EM MÁQUINA SECADORA'},
    {img:'images/lavagem/SECAR-VARAL.jpg',           label:'SECAR EM VARAL, À SOMBRA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
  ],
  SUPLEX_LIGHT:             [
    {img:'images/lavagem/MAX-40-1.jpg',              label:'LAVAR EM TEMPERATURA MÁX. 40°C, SUAVE'},
    {img:'images/lavagem/NAO-ALVEJAR-BRANQUEAR.jpg', label:'NÃO ALVEJAR, NÃO BRANQUEAR'},
    {img:'images/lavagem/SECAR-MIN.jpg',             label:'POSSIVÉL SECAR EM MÁQUINA, TEMPERATURA BAIXA'},
    {img:'images/lavagem/NAO-PASSAR.jpg',            label:'NÃO PASSAR'},
    {img:'images/lavagem/NAO-LAVAR-SECO.jpg',        label:'NÃO LAVAR A SECO'},
    {img:'images/lavagem/UMIDO.jpg',                 label:'LIMPEZA A ÚMIDO'},
  ],
  
  SUPLEX_POWER:             [...WASH_DEFAULT],
  TRILOBAL_BEACH:             [...WASH_DEFAULT],
  TRILOBAL_FIT:               [...WASH_DEFAULT],
  
};
